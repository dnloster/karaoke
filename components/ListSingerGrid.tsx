...
export default function ListSingerGrid() {
    ...
    const { data: topartists, isLoading } = useQuery(["getArtists", gender], () => getArtists(gender));
    ...
    return (
        <>
            <div className="tabs tabs-boxed col-span-full justify-center bg-transparent">
                <div className={`tab ${gender === 1 ? "tab-active" : ""}`} onClick={() => setGender(1)}>
                    Nam
                </div>
                <div className={`tab ${gender === 2 ? "tab-active" : ""}`} onClick={() => setGender(2)}>
                    Nữ
                </div>
                <div className={`tab ${gender === 3 ? "tab-active" : ""}`} onClick={() => setGender(3)}>
                    Nhóm nhạc
                </div>
            </div>
            {isLoading && (
                <>
                    ...
                    {/* {getSkeletonItems(16).map((s) => (
                        <div key={s} className="card bg-gray-300 animate-pulse w-full aspect-w-4 aspect-h-3" />
                    ))} */}
                </>
            )}
            {artist?.map((artist) => {
